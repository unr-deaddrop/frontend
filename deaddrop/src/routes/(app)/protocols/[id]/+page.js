...
export async function load({params}) {
    ...
    const protocol = await fetch(`http://backend.localhost/backend/protocols/${params.id}`, {
        headers: {
    ...
    const endpoints = await fetch(`http://backend.localhost/backend/endpoints/?protocol=${params.id}`, {
        headers: {
            "Content-Type": "application/json"
        },
        method: 'GET'
